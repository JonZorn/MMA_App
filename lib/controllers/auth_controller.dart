...
class AuthController extends GetxController {
  ...
  getSubscription() async {
    try {
      ...
      print(response.data);
      if (response.data == "") {
        return premium.value = false;
      }
      premium.value = response.data['data']['subscriptionId'] != null;
  ...
  createUser(dynamic userData) {
    ...
    return User(
      ...
      name: userData['name'],
      paypalId: userData['paypal_email'],
  ...
  cancelSubscription(
    ...
    try {
      loading.value = true;
      final response = await NetworkClient.post('/cancel/subscription', {
        "reason": reason,
        "feedback": feedback,
      });
      if (response.statusCode == 200) {
  ...
  redeempaypal({String? paypalId, required Function() onSuccess}) async {
    try {
      loading.value = true;
      final response = await NetworkClient.post('/paypal', {
        "paypal_email": paypalId,
      });
      if (response.statusCode == 200) {
        print(response);
  ...
  redeemamzone({String? amount, required Function() onSuccess}) async {
    try {
      loading.value = true;
      final response = await NetworkClient.post('/create-gift-card', {
        "amount": amount,
      });
      if (response.statusCode == 200) {
        print(response);
  ...
  deleteAccount(
      {String? userId, String? password, required Function() onSuccess}) async {
    try {
      loading.value = true;
      final response = await NetworkClient.delete('/delete-user', {
        "user_id": userId,
        "password": password,
      });
      ...
      if (e.response!.statusCode == 400) {
        SnackbarHandler.errorToast(" Invalid Password", "");
      }
